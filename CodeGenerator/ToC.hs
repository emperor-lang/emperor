...
instance ToC ModuleHeader where
    toC c (Module i p) = generatePosLines makeHeaderAndBodyLines c (Module i p) <> makeBodyLines ["#include \"" ++ destFile c ++ ".h\""] <> makeHeaderAndBodyLines ["// This is module " ++ toCString c i ++ " generated by emperor from " ++ (show . sourceFile) c] <> makeHeaderLines ["#pragma GCC dependency " ++ (show . sourceFile) c, "", "#include <banned.h>"]
