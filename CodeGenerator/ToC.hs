...
instance ToC AST where
    toC c (AST m is ms) = toC c m <> (foldr (<>) mempty $ toC c <$> is) <> (foldr (<>) mempty $ toC c <$> ms)

instance ToC ModuleHeader where
    toC c (Module i _) = makeBodyLines ["#include \"" ++ sourceFile c ++ ".h\""] <> makeHeaderAndBodyLines ["// This is module " ++ toCString c i ++ " generated by emperor"]
...
instance ToC FunctionDef where
    toC c (FunctionDef (FunctionTypeDef i t _) is _ _) = prototype <> makeBodyLines [returnString ++ " " ++ toCString c i ++ "(" ++ paramSig ++ ")", "{", "}"]
        where
...
instance ToCString EmperorType where
    ...
    toCString _ BoolP = "bool"
    toCString _ RealP = "real"
...
instance ToC BodyLine where
    toC c (AssignmentC x) = toC c x
    toC c (QueueC x) = toC c x
    toC c (CallC x) = toC c x
...
instance ToC Assignment where
    toC c (Assignment Nothing i e _) = makeBodyLines [makeIndent c ++ show i ++ " " ++ toCString c e]
    toC c (Assignment (Just t) i e _) = makeBodyLines [makeIndent c ++ toCString c t ++ show i ++ " " ++ toCString c e]

instance ToC Queue where
    toC c (Queue Nothing i e _) = makeBodyLines [makeIndent c ++ show i ++ " " ++ toCString c e]
    toC c (Queue (Just t) i e _) = makeBodyLines [makeIndent c ++ toCString c t ++ show i ++ " " ++ toCString c e]

instance ToCString Expr where
    toCString c (Value v _) = toCString c v
    toCString c (Neg e _) = '-' : toCString c e
    toCString c (Add e1 e2 _) = toCString c e1 ++ "+" ++ toCString c e2
    toCString c (Subtract e1 e2 _) = toCString c e1 ++ "-" ++ toCString c e2
    toCString c (Multiply e1 e2 _) = toCString c e1 ++ "*" ++ toCString c e2
    toCString c (Divide e1 e2 _) = toCString c e1 ++ "/" ++ toCString c e2
    toCString c (Modulo e1 e2 _) = toCString c e1 ++ "%" ++ toCString c e2
    toCString c (Less e1 e2 _) = toCString c e1 ++ "<" ++ toCString c e2
    toCString c (LessOrEqual e1 e2 _) = toCString c e1 ++ "<=" ++ toCString c e2
    toCString c (Greater e1 e2 _) = toCString c e1 ++ ">" ++ toCString c e2
    toCString c (GreaterOrEqual e1 e2 _) = toCString c e1 ++ ">=" ++ toCString c e2
    toCString c (Equal e1 e2 _) = toCString c e1 ++ "==" ++ toCString c e2
    toCString c (NotEqual e1 e2 _) = toCString c e1 ++ "!=" ++ toCString c e2
    toCString c (Not e _) = "!" ++ toCString c e
    toCString c (AndStrict e1 e2 _) = toCString c e1 ++ "&" ++ toCString c e2
    toCString c (AndLazy e1 e2 _) = toCString c e1 ++ "&&" ++ toCString c e2
    toCString c (OrStrict e1 e2 _) = toCString c e1 ++ "|" ++ toCString c e2
    toCString c (OrLazy e1 e2 _) = toCString c e1 ++ "||" ++ toCString c e2
    toCString c (Implies e1 e2 _) = '!' : toCString c e1 ++ "||" ++ toCString c e2
    toCString c (Xor e1 e2 _) = toCString c e1 ++ "^" ++ toCString c e2
    toCString c (ShiftLeft e1 e2 _) = toCString c e1 ++ "<<" ++ toCString c e2
    toCString c (ShiftRight e1 e2 _) = toCString c e1 ++ ">>" ++ toCString c e2
    toCString c (ShiftRightSameSign e1 e2 _) = toCString c e1 ++ ">>>" ++ toCString c e2
...
instance ToCString Value where
    ...
    toCString _ (Bool b _) = if b then "true" else "false"
