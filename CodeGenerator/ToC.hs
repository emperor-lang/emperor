...
instance ToC ModuleHeader where
    toC c (Module i p) = makeHeaderAndBodyLines [generatePos c (Module i p)] <> makeBodyLines ["#include \"" ++ destFile c ++ ".h\""] <> makeHeaderAndBodyLines ["// This is module " ++ toCString c i ++ " generated by emperor from " ++ (show . sourceFile) c] <> makeHeaderLines ["#include <banned.h>"]

instance ToC Import where
    toC c (Import l _ _) = toC c l

instance ToC ImportLocation where
    toC c (ImportLocation Global i p) = makeHeaderLines [generatePos c (ImportLocation Global i p), "#include <" ++ toCString c i ++ ">"]
    toC c (ImportLocation Local i p) = makeHeaderLines [generatePos c (ImportLocation Local i p), "#include \"" ++ toCString c i ++ "\""]
...
instance ToC ModuleItem where
    ...
    toC c (FunctionItem f p) = makeHeaderAndBodyLines [generatePos c (FunctionItem f p)] <> toC c f
