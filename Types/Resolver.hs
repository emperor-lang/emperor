...
instance Typable Expr where
    ...
    g |> (Not e)                    = let t = g |> e in
                                        assert (t == Valid BoolP) ("Could not unify Bool and " ++ show t) t
    ...
    g |> (Set (e:es))               = let t = g |> e in
                                        assert (all (== t) ((g |>) <$> es)) "All elements of a set must have the same type" (
                                            case t of 
    ...
    g |> (Tuple es)                 = if all validType tjs
                                        then
                                            Valid (ETuple ts)
                                        else
...
assertEquality :: Typable a => Typable b => TypeEnvironment -> a -> b -> TypeJudgementResult
assertEquality g e1 e2 = let t1 = g |> e1 in
                            case t1 of
                                Valid t1' -> 
                                    let t2 = g |> e2 in
                                        case t2 of
...
unbox (Invalid _:_) = error $ "Failed assertion that all of " ++ show tjs ++ " are valid type judgements"
unbox (Valid t:tjs') = t : unbox tjs'

validType :: TypeJudgementResult -> Bool
validType (Valid _) = True
validType (Invalid _) = False
