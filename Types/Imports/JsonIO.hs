...
import           Data.Aeson             (FromJSON, ToJSON, Value (Object), eitherDecode', encode, object, parseJSON,
                                         toJSON, (.:), (.=))
import           Data.ByteString.Lazy   (readFile, writeFile)
import           Logger.Logger          (Loggers)
import           Parser.AST             (ImportLocation (..))
...
data Header =
    Header String [ImportLocation] TypeEnvironment
    deriving (Show)

instance ToJSON Header where
    toJSON (Header s ds g) = object ["name" .= s, "depends" .= ds, "environment" .= g]

instance FromJSON Header where
    parseJSON (Object v) =
        Header <$> v .: "name" <*> v .: "depends" <*> v .: "environment"
