...
-}
module Types.Imports.JsonIO (Header(..), readHeader, writeHeader) where

import AST (Ident(Ident), ImportLocation(..), ImportType)
import Data.Aeson (FromJSON, Value(Object), ToJSON, (.=), (.:), eitherDecode', encode, object, parseJSON, toJSON)
import Data.ByteString.Lazy (readFile, writeFile)
import Prelude hiding (readFile, writeFile)
import Types.Environment (TypeEnvironment(..))

data Header = Header Ident [ImportLocation] TypeEnvironment
    deriving Show
...
instance FromJSON Header where
    parseJSON (Object v) = Header <$> (Ident <$> v .: "name")
                                  <*> v .: "depends"
                                  <*> v .: "environment"
...
readHeader p = do
    c <- readFile p
    let r = eitherDecode' c :: Either String Header
    return r
